...
class StaticGalaxyInformationPlot:
    ...
    def make_plot(self):
        ...
        keys = [key for key in self.galaxy_data["planet_type_distribution"].keys() if key not in timeline.COLONIZABLE_PLANET_CLASSES]
        keys = sorted(keys, key=lambda x: self.galaxy_data["planet_type_distribution"][x])
        values = [self.galaxy_data["planet_type_distribution"].get(key, 0) for key in keys]
        ...
        keys = [key for key in timeline.COLONIZABLE_PLANET_CLASSES if key in self.galaxy_data["planet_type_distribution"]]
        ...
        values = [self.galaxy_data["planet_tiles_distribution"].get(key, 0) / self.galaxy_data["planet_type_distribution"].get(key, 0) for key in keys]
        colors = [color_dict[timeline.CLIMATE_CLASSIFICATION[key]] for key in keys]
        ax.bar(range(len(values)), values, tick_label=keys, color=colors)

    def _sort_planets_by_climate_and_frequency(self, planet_class):
        total_planet_count = sum(self.galaxy_data["planet_type_distribution"].values())
        return total_planet_count * timeline.PLANET_CLIMATES.index(timeline.CLIMATE_CLASSIFICATION[planet_class]) + self.galaxy_data["planet_type_distribution"].get(planet_class, 0)
...
class EmpireDemographicsPlot:
    def __init__(self, empire_data, plot_filename="./output/empire_demographics_plot.png"):
